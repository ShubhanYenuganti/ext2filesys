...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group = BLOCK_SIZE * 8; // each bit represents one block
...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = 2; // . and .. directories are always there by default
...
void write_block_bitmap(int fd)
{
	...
	// The goal for the block bitmap is to specify that 24-1023 are free, and everything else is unavailable
	// 0-15 are unavailable
	// 23 is available, 16 - 22 is unavailable
	// 24 - 1015 is available
	// 1023 is unavailable, 1016 - 1022 are available
	// 1024+ are unavailable

	u8 map_value[BLOCK_SIZE] = {0}; // 0 -- free, 1 -- unavailable

	map_value[0] = 0b11111111; // 0 -- 0-8
	map_value[1] = 0b11111111; // 0 -- 8-15
	map_value[2] = 0b01111111; // 16-22 are unavailable, 23 is available
	map_value[127] = 0b10000000; // 1023 is unavailable, 1016-1022 are available
...
void write_inode_bitmap(int fd)
{
	...
	// 0-13 are unavailable inodes (115 free inodes)
	map_value[0] = 0b11111111;
	map_value[1] = 0b00011111;

	// inodes 14-128 are free
	// remaining aren't free
...
void write_inode_table(int fd) {
	...
	// Same permissions as lost_and_found d rwx r-x r-x
	...
	root_inode.i_gid = 0;
	root_inode.i_links_count = 3; // ., .. and the root directory itself
	...
	memcpy( (char*) hello_inode.i_block, "hello-world", 12);
...
void write_root_dir_block(int fd)
{
	...
	// at each step track remaining bits
	...
	struct ext2_dir_entry current_dir = {0};
	dir_entry_set(current_dir, EXT2_ROOT_INO, ".");
	dir_entry_write(current_dir, fd);

	remaining_bytes -= current_dir.rec_len;

	struct ext2_dir_entry parent_dir = {0};
	dir_entry_set(parent_dir, EXT2_ROOT_INO, "..");
	dir_entry_write(parent_dir, fd);

	remaining_bytes -= parent_dir.rec_len;

	struct ext2_dir_entry hello_entr = {0};
	dir_entry_set(hello_entr, HELLO_INO, "hello");
	dir_entry_write(hello_entr, fd);

	remaining_bytes -= hello_entr.rec_len;

	struct ext2_dir_entry hello_world_entr = {0};
	dir_entry_set(hello_world_entr, HELLO_WORLD_INO, "hello-world");
	dir_entry_write(hello_world_entr, fd);

	remaining_bytes -= hello_world_entr.rec_len;

	struct ext2_dir_entry lost_found_entr =  {0};
	dir_entry_set(lost_found_entr, LOST_AND_FOUND_INO, "lost+found");
	dir_entry_write(lost_found_entr, fd);

	remaining_bytes -= lost_found_entr.rec_len;

	// Root Directory End
	struct ext2_dir_entry fill_entry = {0};
	fill_entry.rec_len = remaining_bytes;
...
void write_hello_world_file_block(int fd)
{
	...
	if (write(fd, "Hello world\n", 12) != 12) {
		errno_exit("Hello World file write error");
	}
